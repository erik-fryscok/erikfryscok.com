...
| 2026-07-23 | Manage planning model-selection guidance via a Cline workspace rule (`.clinerules/`), scoped to direct creator APIs on pay-per-use terms for cost tracking; route by task reasoning load across provider families and tiers rather than pinned model versions. | Rules are always-on (unlike on-demand Skills) and Cline-native; workspace scope keeps it version-controlled and shared. Direct creator APIs (Anthropic, OpenAI, Google, z.ai, DeepSeek, Mistral) preserve per-call cost visibility; subscription/usage-billing aggregators and third-party open-weight hosters are excluded. Routing by family and tier (not version) keeps the guidance valid as models release. | Active |
